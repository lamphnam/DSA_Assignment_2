...
std::ostream &xoutput(std::ostream &out, const E &e, xstrided_slice_vector &slices, F &printer, std::size_t blanks,
...
struct printer<T, std::enable_if_t<std::is_floating_point<typename T::value_type>::value>> {
    ...
    explicit printer(std::streamsize precision) : m_precision(precision) {}
...
template <class T>
struct printer<T, std::enable_if_t<xtl::is_integral<typename T::value_type>::value &&
...
struct printer<T, std::enable_if_t<xtl::is_complex<typename T::value_type>::value>> {
    ...
    explicit printer(std::streamsize precision) : real_printer(precision), imag_printer(precision) {}
...
template <class T>
struct printer<T, std::enable_if_t<!xtl::is_fundamental<typename T::value_type>::value &&
...
public:
    explicit fmtflags_guard(S &stream) : m_stream(stream), m_flags(stream.flags()) {}
