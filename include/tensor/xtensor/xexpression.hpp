...
template <class D>
inline xsharable_expression<D>::xsharable_expression() : p_shared(nullptr) {}
...
struct is_crtp_base_of_impl<B, F<E>> : xtl::disjunction<std::is_base_of<B<E>, F<E>>, std::is_base_of<B<F<E>>, F<E>>> {};
} // namespace detail
...
#define XTENSOR_FORWARD_CONST_ITERATOR_METHOD(name)                                                                    \
    ...
    auto name(const S &shape) const noexcept -> decltype(std::declval<xtl::constify_t<E>>().template name<L>(shape)) { \
        return m_ptr->template name<L>();                                                                              \
    }
...
public:
    ...
    using strides_type =
        xtl::mpl::eval_if_t<has_strides<E>, detail::expr_strides_type<E>, get_strides_type<shape_type>>;
    using backstrides_type =
        xtl::mpl::eval_if_t<has_strides<E>, detail::expr_backstrides_type<E>, get_strides_type<shape_type>>;
    using inner_strides_type =
        xtl::mpl::eval_if_t<has_strides<E>, detail::expr_inner_strides_type<E>, get_strides_type<shape_type>>;
    using inner_backstrides_type =
        xtl::mpl::eval_if_t<has_strides<E>, detail::expr_inner_backstrides_type<E>, get_strides_type<shape_type>>;
    ...
    template <class S>
    auto stepper_end(const S &shape,
                     layout_type l) const noexcept -> decltype(std::declval<const E>().stepper_end(shape, l)) {
 ...
 */
template <class E>
inline xshared_expression<E>::xshared_expression(const std::shared_ptr<E> &ptr) : m_ptr(ptr) {}
