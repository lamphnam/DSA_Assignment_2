...
protected:
  ...
  // keys stored in pMap
  void (*deleteValues)(xMap< K, V > *); // deleteValues(xMap<K,V>* pMap):
  // delete all values stored in pMap

public:
  xMap(int (*hashCode)(K &, int), // require
       float loadFactor = 0.75f, bool (*valueEqual)(V &, V &) = 0, void (*deleteValues)(xMap< K, V > *) = 0,
       bool (*keyEqual)(K &, K &) = 0, void (*deleteKeys)(xMap< K, V > *) = 0);
   ...
   */
  static int intKeyHash(int &key, int capacity) { // HashFunction
    return key % capacity;
  }
...
protected:
  ...
  void moveEntries(DLinkedList< Entry * > *oldTable, int oldCapacity, DLinkedList< Entry * > *newTable,
                   int newCapacity);
...
template < class K, class V > xMap< K, V >::xMap(const xMap< K, V > &map) {
  ...
  for (int i = 0; i < map.capacity; i++) {
    for (Entry *entry : map.table[i]) {
      ...
      this->table[i].add(newEntry);
    }
  }
}

template < class K, class V > xMap< K, V > &xMap< K, V >::operator=(const xMap< K, V > &map) {
  if (this == &map) {
    return *this; // Xử lý tự gán
  }

  // Dọn dẹp tài nguyên hiện tại
  for (int i = 0; i < this->capacity; i++) {
    for (Entry *entry : this->table[i]) {
      delete entry; // Giả sử quản lý bộ nhớ đúng trong Entry
    }
  }
  delete[] this->table; // Dọn dẹp mảng của DLinkedLists
  ...
  for (int i = 0; i < map.capacity; i++) {
    for (Entry *entry : map.table[i]) {
      Entry *newEntry = new Entry(*entry); // Sao chép mục
...
template < class K, class V > xMap< K, V >::~xMap() {
  // First, call user-defined deletion functions if they exist
  if (deleteValues != 0)
    deleteValues(this);
  if (deleteKeys != 0)
    deleteKeys(this);

  // Clean up each bucket in the hash table
  for (int idx = 0; idx < capacity; idx++) {
    DLinkedList< Entry * > &list = table[idx];

    // Use iterator to access entries
    typename DLinkedList< Entry * >::Iterator it;
    for (it = list.begin(); it != list.end(); it++) {
      Entry *entry = *it;
      delete entry; // Free the entry object
    }
...
template < class K, class V > V xMap< K, V >::put(K key, V value) {
  ...
  // Nếu không tìm thấy key, tạo một Entry mới
  Entry *newEntry = new Entry(key, value);
  list.add(newEntry);
  count++;
  ensureLoadFactor(count); // Truyền count vào ensureLoadFactor
  return retValue;
}

template < class K, class V > V &xMap< K, V >::get(K key) {
...
template < class K, class V > V xMap< K, V >::remove(K key, void (*deleteKeyInMap)(K)) {
  int index = hashCode(key, capacity);
  DLinkedList< Entry * > &list = table[index];
...
template < class K, class V > bool xMap< K, V >::containsKey(K key) {
  int index = hashCode(key, capacity);
  DLinkedList< Entry * > &list = table[index];
...
template < class K, class V > bool xMap< K, V >::containsValue(V value) {
  for (int i = 0; i < this->capacity; i++) {
...
template < class K, class V > string xMap< K, V >::toString(string (*key2str)(K &), string (*value2str)(V &)) {
  stringstream os;
  string mark(50, '=');
  os << mark << endl;
 ...
 * "loadFactor*capacity"
 */
template < class K, class V > void xMap< K, V >::ensureLoadFactor(int current_size) {
...
template < class K, class V > void xMap< K, V >::copyMapFrom(const xMap< K, V > &map) {
  removeInternalData();
