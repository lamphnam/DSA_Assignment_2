...
public:
    xMap(int (*hashCode)(K &, int),  // require
         float loadFactor = 0.75 f, bool (*valueEqual)(V &, V &) = 0, void (*deleteValues)(xMap< K, V > *) = 0,
...
xMap< K, V >::~xMap() {
    ...
    for (int i = 0; i < this->capacity; i++) {
        ...
        for (auto node = list.head; node != nullptr;) {
    ...
    delete[] this->table;  // Giải phóng bộ nhớ cho bảng
}
...
V xMap< K, V >::put(K key, V value) {
    ...
    for (auto node = list.head; node != nullptr; node = node->next) {
        if (keyEQ(node->data->key, key)) {
            retValue = node->data->value;
...
V xMap< K, V >::remove(K key, void (*deleteKeyInMap)(K)) {
    ...
    for (auto node = list.head; node != nullptr; node = node->next) {
        if (keyEQ(node->data->key, key)) {
            V retValue = node->data->value;
            if (deleteKeyInMap) {
                deleteKeyInMap(node->data->key);
            }
            list.removeItem(node->data, deleteEntry);
            return retValue;
        }
    }
...
bool xMap< K, V >::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V)) {
    ...
    for (auto node = list.head; node != nullptr; node = node->next) {
        if (keyEQ(node->data->key, key) && valueEQ(node->data->value, value)) {
            if (deleteKeyInMap) {
                deleteKeyInMap(node->data->key);
            }
            if (deleteValueInMap) {
                deleteValueInMap(node->data->value);
            }
            list.removeItem(node->data, nullptr);
            return true;
        }
    }
...
bool xMap< K, V >::containsValue(V value) {
    ...
    for (int i = 0; i < this->capacity; i++) {
        DLinkedList< Entry * > &list = this->table[i];
        for (auto node = list.head; node != nullptr; node = node->next) {
            if (valueEQ(node->data->value, value)) {
                return true;
            }
        }
    }
