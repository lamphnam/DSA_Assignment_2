...
template <class T> XArrayList<T>::~XArrayList() {
  // TODO
  delete[] data;
}
...
template <class T> int XArrayList<T>::size() {
  // TODO
  return count;
}
...
template <class T> string XArrayList<T>::toString(string (*item2str)(T &)) {
  /**
   * Converts the array list into a string representation, formatting each
   * element using a user-defined function. If no function is provided, it uses
   * the default string representation for each element. Example: Given an array
   * list with elements {1, 2, 3} and a function that converts integers to
   * strings, calling toString would return "[1, 2, 3]".
   *
   * @param item2str A function pointer for converting items of type T to
   * strings. If null, default to the string conversion of T.
   * @return A string representation of the array list with elements separated
...
template <class T> void XArrayList<T>::ensureCapacity(int index) {
  /**
  ...
  if (index >= capacity) {
    ...
    try {
      ...
      throw std::runtime_error(
          "Memory allocation failed while resizing the array");
    }
  }
}
