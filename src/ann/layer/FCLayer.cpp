...
xt::xarray<double> FCLayer::forward(xt::xarray<double> X) {
   // std::cout<<"FCLayer:__________" <<endl;
    if (m_trainable) {
        m_aCached_X = X;
    }
    
    xt::xarray<double> Y = xt::linalg::tensordot(X, xt::transpose(m_aWeights), {1}, {0});
    //std::cout << "Y shape after tensordot: {" << Y.shape()[0] << ", " << Y.shape()[1] << "}" << std::endl;

    if (m_bUse_Bias) {
        // Use view with newaxis to reshape bias from (Nout,) to (1, Nout)
        auto bias_view = xt::view(m_aBias, xt::newaxis(), xt::all());
        Y = Y + bias_view;
    }
    return Y;
}
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY) {
    ...
    // Calculate gradient for weights (ΔW)
    m_aGrad_W += xt::linalg::tensordot(xt::transpose(DY), m_aCached_X, {1}, {0});
    ...
    // Calculate gradient for input (ΔX)
    auto DX = xt::linalg::tensordot(DY, m_aWeights, {1}, {0});
