...
FCLayer::FCLayer(string sParams, string filename_w, string filename_b, string sName) {
    ...
    try {
        ...
        if(weight_file_invalid) {
            ...
            bool valid = (W.dimension() == 2) && (W.shape()[0] == m_nNout) && (W.shape()[1] == m_nNin);
            if(!valid) {
        ...
        if(bias_file_invalid) {
            ...
            if(m_bUse_Bias) {
                ...
                if(!valid) throw std::runtime_error("FC::Bias: shape from data is not the same with the specification");
...
string FCLayer::get_desc() {
    string desc = fmt::format("{:<10s}, {:<15s}: {:<4d}, {:<4d}, {:<4d}", "FC", this->getname(), this->m_nNin,
                              this->m_nNout, this->m_bUse_Bias);
    return desc;
}
