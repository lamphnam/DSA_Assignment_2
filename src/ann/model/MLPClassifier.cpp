...
double_tensor MLPClassifier::predict(DataLoader<double, double> *pLoader, bool make_decision) {
    ...
    for(auto batch : *pLoader) {
        ...
        if(first_batch) {
            ...
            // Nối kết quả vào results
            results = xt::vstack(xt::xtuple(results, Y));
        }
    }
...
double_tensor MLPClassifier::evaluate(DataLoader<double, double> *pLoader) {
    ...
    for(auto batch : *pLoader) {
        ...
        // Forward pass để có prediction
        double_tensor Y = forward(X);

        // Chuyển về nhãn dự đoán và nhãn thực
...
void MLPClassifier::backward() {
    // Bắt đầu với CrossEntropy (hoặc bất kỳ lớp nào cần tính DY đầu tiên)
    ILossLayer *lossLayer = dynamic_cast<ILossLayer *>(m_layers.get(m_layers.size() - 1));

    // Kiểm tra con trỏ lossLayer, nếu không hợp lệ thì bỏ qua
    if(lossLayer) {
        double_tensor DY = lossLayer->backward(); // Tính DY từ CrossEntropy

        // Duyệt qua các lớp còn lại từ lớp thứ hai đến lớp đầu tiên
        for(int i = m_layers.size() - 2; i >= 0; --i) {
            ILayer *layer = m_layers.get(i);
            if(layer) {
                // Gọi backward cho từng lớp và truyền DY vào
                DY = layer->backward(DY);
            }
            // Nếu layer là nullptr, chỉ bỏ qua mà không làm gián đoạn chương trình
        }
    }
    // Nếu không có lớp loss hợp lệ, không làm gì cả (bỏ qua)
}
...
bool MLPClassifier::save(string model_path) {
    try {
        ...
        // Chỗ này file gốc là string arch_file = fs::path(model_path) / fs::path(m_pConfig->get("arch_file",
        // "arch.txt")); lúc nộp bài check lại
        ...
        string message = fmt::format("MLPClassifier::save: failed; model_path={:s}", model_path);
        cerr << message << endl;
        cerr << e.what() << endl;
        return false;
    }
