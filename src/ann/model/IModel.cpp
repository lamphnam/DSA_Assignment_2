...
void IModel::on_begin_training(DataLoader<double, double> *pTrainLoader, DataLoader<double, double> *pValidLoader,
...
void IModel::on_end_step(double batch_loss) {
    ...
    string message =
        fmt::format("{:3d}/{:3d}|{:4d}| {:6.2f} {:6.2f} | {:6.2f}", m_current_epoch, m_nepoches, m_current_batch,
                    batch_loss, m_epoch_loss / m_sample_counter, train_metrics[ulong(ACCURACY)]);
    cout << message << endl;
}